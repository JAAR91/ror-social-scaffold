module FriendshipsHelper
  def friend_user_links(user)
    ...
    if asn.nil?
      path = user_friendships_path(user_id: current_user.id, friend_id: user.id, status: 'pending',
                                   location: users_path)
      link_to('Add Friend', path, method: :post, class: 'btn btn-outline-info btn-sm')
      ...
      link_to('Delete Friend!', invitedecline_path(id: asn.id, location: users_path, friend: 'delete'),
